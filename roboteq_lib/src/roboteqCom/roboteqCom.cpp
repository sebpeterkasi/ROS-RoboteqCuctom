...
int    RoboteqCom::ReadReply(string& reply)
{
    ...
    else
    {
		...
		boost::split(fields, reply, boost::algorithm::is_any_of("S"));
		static ros::NodeHandle n;
		static ros::Publisher velocity_pub = n.advertise<std_msgs::Float32>("velocity", 1000);
		static std_msgs::Float32 speed;

		if(fields.size()==2){
			string temp2 = string(fields[1]);
			if(temp2.size()==4){
				if(reply.size()>=6){
					string temp = string(fields[0]);
	    				speed.data = atoi(temp.c_str())*0.006283184;
